...
void MCP79410::set_sys_time(RTCC_Data new_time){
	...
	this->write(ADDR_DAY,day|VBATEN); //enable battery
	this->write(ADDR_CTRL,SQWE+ALM_NO+MFP_01H);//
	this->write(dec2bcd(new_time.min),SET_MIN);
	this->write(dec2bcd(new_time.hour),SET_HOUR);
	this->write(dec2bcd(new_time.date),SET_DATE);
	this->write(dec2bcd(new_time.month),SET_MONTH);
	this->write(dec2bcd(new_time.year),SET_YEAR);
	this->write(ADDR_SEC,START_32KHZ);
}
